...
function connectScesiChain() {
    const cmd = spawn('multichaind', ['scesichain','-daemon']);
...
function registerScesiChain() {
    const cmd = spawn('multichaind', ['scesichain@200.58.83.81:2649']);
...
function consultarSaldo(){
    const cmd = spawn('multichain-cli', ['scesichain', 'gettotalbalances']);
    cmd.stdout.on('data', (data) => {
        console.log(`stdout: ${data}`);
        let buffer = data.toString();
...
//let object = JSON.parse(data);
    const cmd = spawn('multichain-cli', ['scesichain','sendassettoaddress',data.cuenta,'ScesiCoin',data.monto]);
    cmd.stdout.on('data', (data) => {
        console.log(`stdout: ${data}`);
    });
